...
const validateCEP = async (cep: string) => {
	...
	try {
		...
		const viaCepResponse = await retry(() => axios.get(`https://viacep.com.br/ws/${cep}/json/`), 3, 400);
		if (viaCepResponse.data.erro) {
			throw new Error("CEP não encontrado");
		}

		// Build address variants to improve geocoding success rate
		const addressBase = viaCepResponse.data;
		const variants: string[] = [];
		if (addressBase.logradouro) variants.push(`${addressBase.logradouro}, ${addressBase.localidade} ${addressBase.uf}, Brazil`);
		if (addressBase.bairro) variants.push(`${addressBase.bairro}, ${addressBase.localidade} ${addressBase.uf}, Brazil`);
		variants.push(`${addressBase.localidade} ${addressBase.uf}, Brazil`);
		variants.push(`${cep}, ${addressBase.localidade} ${addressBase.uf}, Brazil`);

		// 2. Get coordinates from Nominatim trying different queries
		const nominatimHeaders = { 'User-Agent': 'AgenteEscola/1.0 (contato@seusite.com)', Referer: window?.location?.origin || '' };
			...
			for (const q of variants) {
				try {
					const res = await axios.get(
						...
						{ headers: nominatimHeaders, timeout: 8000 }
					);
		...
		const nominatimResult = await retry(() => tryGeocode(), 3, 500);
		const lat = parseFloat(nominatimResult.lat);
		const lon = parseFloat(nominatimResult.lon);

		// 3. Search for schools using Overpass API (request center for ways/relations)
		const overpassQuery = `[out:json][timeout:25];(node["amenity"="school"](around:10000,${lat},${lon});way["amenity"="school"](around:10000,${lat},${lon});relation["amenity"="school"](around:10000,${lat},${lon}););out center;`;

		const overpassResponse = await retry(
		...
		// Process schools: support node (lat/lon) and way/relation center
		...
		const schools = elements
			...
			.map((element: any) => {
				...
				return {
					...
					distance: elLat && elLon ? calculateDistance(lat, lon, elLat, elLon) : Number.MAX_VALUE,
				} as School;
			})
			.filter((s: School) => isFinite(s.distance))
